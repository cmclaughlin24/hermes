...
describe('OrmApiKeyRepository', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OrmApiKeyRepository,
        {
          ...
          useValue: createMockRepository<ApiKeyEntity>(),
        },
      ],
    }).compile();

    repository = module.get<OrmApiKeyRepository>(OrmApiKeyRepository);
    apiKeyModel = module.get<MockRepository>(getRepositoryToken(ApiKeyEntity));
  });
  ...
  describe('findById()', () => {
    ...
    it('should yield an api key', async () => {
      // Arrange.
      const expectedResult: ApiKeyEntity = {
        id: randomUUID(),
        name: 'space-marine-2',
        expiresAt: new Date(),
        createdBy: 'lt. titus',
  ...
  describe('create()', () => {
    ...
    it('should create an api key', async () => {
      // Assert.
      apiKeyModel.save.mockResolvedValue({} as ApiKeyEntity);
